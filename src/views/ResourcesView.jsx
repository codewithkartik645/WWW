...
import {
  C, uid, deptName, activeDepartments,
} from "../theme";
import {
  } from "../data/seedData";
...
function ResourcesView({ data, setData, isAdmin }) {
  ...
  const myDeptId = (isHOD ? myProfile?.departmentId : myProfile?.departmentId) || data.departments[0]?.id;
  const hasDepts = data.departments.length > 1;

  const [mode, setMode] = useState("file");
  const [form, setForm] = useState({ title: "", url: "", courseId: "" });
  const [postTo, setPostTo] = useState(isDirector ? "all" : myDeptId); // Director can share campus-wide; everyone else tags their own department
  const [viewTab, setViewTab] = useState(isDirector ? "all" : myDeptId);
  ...
  const add = () => {
    const departmentId = isDirector ? (postTo === "all" ? null : postTo) : myDeptId;
    if (mode === "file") {
      if (!pending) return;
      setData((d) => logActivity({ ...d, resources: [...d.resources, { id: uid(), title: form.title || pending.name, fileName: pending.name, fileType: pending.type, dataUrl: pending.dataUrl, courseId: form.courseId || null, departmentId }] }, `Resource uploaded: ${form.title || pending.name}`));
      setPending(null); if (fileRef.current) fileRef.current.value = "";
    } else {
      if (!form.title.trim() || !form.url.trim()) return;
      setData((d) => logActivity({ ...d, resources: [...d.resources, { id: uid(), title: form.title, url: form.url, courseId: form.courseId || null, departmentId }] }, `Resource linked: ${form.title}`));
    }
    setForm({ title: "", url: "", courseId: "" });
  };
  const [confirmDelete, deleteModal] = useDeleteConfirm();
  // Nobody can remove a resource outside their own department — a campus-wide (departmentId
  // null) one can only be removed by the Director who has that broader view.
  const canManage = (r) => isDirector || r.departmentId === myDeptId;
  const remove = (id) => {
  ...
  // Own department's resources plus every campus-wide one. The Director additionally gets a
  // tab strip to narrow the list down to one department (or see everything with "All").
  const visible = useMemo(() => data.resources.filter((r) => {
    if (isDirector) return viewTab === "all" || r.departmentId === viewTab;
    return !r.departmentId || r.departmentId === myDeptId;
  }), [data.resources, isDirector, viewTab, myDeptId]);
      ...
      <Card className="mb-4">
        ...
        {isHOD && hasDepts && (
          <div className="text-xs mb-2 px-2.5 py-1.5 rounded-lg w-fit" style={{ background: "#F6F0E4", color: "#6E6455" }}>
            Shared with <b>{deptName(data, myDeptId)}</b> only
          </div>
        )}
        {!isAdmin && hasDepts && (
          <div className="text-xs mb-2 px-2.5 py-1.5 rounded-lg w-fit" style={{ background: "#F6F0E4", color: "#6E6455" }}>
            Shared with your department, <b>{deptName(data, myDeptId)}</b>
          </div>
        )}
        {isDirector && hasDepts && (
            ...
            <Card key={r.id} className="flex items-center justify-between !py-3">
              ...
              {r.dataUrl ? (
                <button type="button" onClick={() => openAttachment(r.dataUrl, r.fileName || r.title, r.fileType)} className="flex items-center gap-2 text-sm" style={{ color: C.purple }}>
                  ...
                  {isAdmin && hasDepts && <span className="text-[9.5px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: r.departmentId ? "#F6F0E4" : "#EAEEF0", color: r.departmentId ? "#6E6455" : "#2C4A63" }}>{r.departmentId ? deptName(data, r.departmentId) : "All Depts"}</span>}
                </button>
              ) : (
              ...
              <div className="flex items-center gap-2">
                ...
                {canManage(r) && <button onClick={() => remove(r.id)} className="text-[#D9D0BC] hover:text-[#A6423A]"><Trash2 size={15} /></button>}
              </div>
            </Card>
          );
        })}
        {visible.length === 0 && <div className="text-sm text-[#A79E8C]">No resources saved yet.</div>}
      </div>
    </div>
  );
}
