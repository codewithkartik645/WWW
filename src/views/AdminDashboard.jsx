...
import {
  CalendarDays, BookOpen, ListChecks, Wand2, ClipboardList, Users2, Building2, ChevronRight, UserCheck,
} from "lucide-react";
...
function AdminDashboard({ data, setData, goTo }) {
  ...
  const scopedAttendance = isHOD ? data.attendance.filter((a) => a.departmentId === myDeptId) : data.attendance;
  const todayStr = new Date().toISOString().slice(0, 10);
  const todaysAttendance = scopedAttendance.filter((a) => a.date === todayStr);
  const attendancePctToday = todaysAttendance.length ? Math.round((todaysAttendance.filter((a) => a.status === "present").length / todaysAttendance.length) * 100) : null;
  ...
  const stats = [
    ...
    { label: "Attendance Today", value: attendancePctToday === null ? "—" : `${attendancePctToday}%`, icon: UserCheck, color: "#2C4A63" },
  ...
  const departmentSnapshots = useMemo(() => {
    ...
    return data.departments.map((dp) => {
      ...
      const deptAttendance = data.attendance.filter((a) => a.departmentId === dp.id);
      const deptAttendancePct = deptAttendance.length ? Math.round((deptAttendance.filter((a) => a.status === "present").length / deptAttendance.length) * 100) : null;
      return {
        dept: dp,
        ...
        attendancePct: deptAttendancePct,
      };
    });
  }, [isDirector, data]);
      ...
      {isDirector && hasDepts && (
        <Card className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {departmentSnapshots.map(({ dept, studentCount, courseCount, hodName, upcomingExams, attendancePct }) => (
              <button
                key={dept.id}
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  ...
                  {attendancePct !== null && <span className="text-[11px] px-2 py-1 rounded-full" style={{ background: "#EAEEF0", color: "#2C4A63" }}>{attendancePct}% attendance</span>}
                </div>
